...
security_agent = ToolCallingAgent(
    tools=[],
    model=model,
    name="security_agent",
    description="Analyzes security requirements, develops confidentiality protocols, and evaluates researcher credentials",
    max_steps=8,
    verbosity_level=1
)

researcher_screening_agent = ToolCallingAgent(
    tools=[DuckDuckGoSearchTool(), VisitWebpageTool()],
    model=model,
    name="researcher_screening_agent",
    description="Searches for and evaluates potential research candidates based on qualifications, publications, and compatibility",
    max_steps=12,
    planning_interval=2,
    verbosity_level=1
)

resource_planning_agent = ToolCallingAgent(
    tools=[],
    model=model,
    name="resource_planning_agent",
    description="Plans laboratory setup, equipment requirements, supply chain logistics, and resource allocation",
    max_steps=10,
    planning_interval=2,
    verbosity_level=1
)

contract_agent = ToolCallingAgent(
    tools=[],
    model=model,
    name="contract_agent",
    description="Drafts smart contracts, negotiation terms, and legal agreements for research collaboration",
    max_steps=8,
    verbosity_level=1
)

facility_design_agent = ToolCallingAgent(
    tools=[image_generation_tool],
    model=model,
    name="facility_design_agent",
    description="Creates facility layouts, housing plans, and research station designs for the island",
    max_steps=10,
    planning_interval=2,
    verbosity_level=1
)
...
manager_agent = CodeAgent(
    tools=[],
    model=model,
    managed_agents=[security_agent, researcher_screening_agent, resource_planning_agent, contract_agent, facility_design_agent],
    name="manager_agent",
